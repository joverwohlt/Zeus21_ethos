...
class Power_Spectra_Binned:
    def __init__(self, Power_Spectra, T21_coefficients, k_range,z_range):
        ...
        self.klist = Power_Spectra.klist_PS
        self.zlist = T21_coefficients.zintegral


        for i in range(len(self.z_center_value)):
            self.z_center_value[i] = (z_range[i]-z_range[i+1])/2+z_range[i+1]

        for i in range(len(self.k_center_value)):
            self.k_center_value[i] = (k_range[i]-k_range[i+1])/2+k_range[i+1]
        ...
        for j in range(len(self.z_center_value)):
            iz_list.append(min(range(len(self.zlist)), key=lambda i: np.abs(self.zlist[i]-self.z_center_value[j])))
        for j in range(len(self.k_center_value)):
            ik_list.append(min(range(len(self.klist)), key=lambda i: np.abs(self.klist[i]-self.k_center_value[j])))
...
def log_posterior(theta, data, noise, k_range, z_range, h_peak_range, k_peak_range, 
                 hmf_dict, CosmoParams, ClassCosmo, Correlations, 
                 epsstar = 0.1, alphastar = 0.5, L40_xray = 3.0, E0_xray = 500, z=10, RSD=1, Flag_Only_ETHOS = True):
...
def log_prior(h_peak, k_peak,h_peak_range, k_peak_range, epsstar_range, alphastar_range, L40_xray_range, E0_xray_range,
              epsstar = 0.1, alphastar = 0.5, L40_xray = 3.0, E0_xray = 500):
